  ...
  <div>
    <div>
      <el-row>
        <el-col :span="8">
          <el-cascader
            v-model="tagslistQuery.tagsSelTeacher"
            :options="tagsTeacherOptions"
            placeholder="选择老师" 
            clearable
        ...
        <el-col :span="8">
          <el-select v-model="tagslistQuery.tagsAcademicyear" placeholder="学年" clearable 
            style="margin-left:7px;margin-top:15px" class="filter-item">
            <el-option v-for="item in tagsAcademicyearOptions" :key="item.index" :label="item.label" :value="item.value" />
          </el-select> 
        </el-col>
        <el-col :span="4">
          <el-select v-model="tagslistQuery.tagsAcademicterm" placeholder="学期" clearable
            style="margin-left:7px;margin-top:15px" class="filter-item">
            <el-option v-for="item in tagsAcademictermOptions" :key="item.index" :label="item.label" :value="item.value" />
          </el-select> 
        </el-col>
        <el-col :span="4">
...
export default {
  ...
  data() {
    return {
      ...
      tagsTeacherOptions:[],
      tagsAcademicyearOptions:[],
      tagsAcademictermOptions:[],
  ...
  mounted() {
    ...
    for(let i = yearNow+1 ; i > yearNow-10 ; i--){
      ...
      this.tagsAcademicyearOptions.push(obj)
    }
    ...
    for(let i = 1 ; i <=2 ; i++){ 
      ...
      this.tagsAcademictermOptions.push(obj)
    } 
    this.initChart()
  },
  beforeDestroy() {
  ...
  methods: { 
    changeData() {
      /**
        if(this.tagslistQuery.tagsAcademicyear == null || this.tagslistQuery.tagsAcademicyear == ""){
          this.$message({
            showClose: true,
            message: '您可以选择教学学年' 
          });
        }
        if(this.tagslistQuery.tagsAcademicterm == null || this.tagslistQuery.tagsAcademicterm == ""){
          this.$message({
            showClose: true,
            message: '您可以选择教学学期', 
          });
       ...
       */
      if(this.tagslistQuery.tagsSelTeacher == null || this.tagslistQuery.tagsSelTeacher == "" ){
        this.$message({
          showClose: true,
          message: '对不起，请选择教师！',
          type: 'error'
        ...
        const res = (async function() { 
          var teacherid = that.tagslistQuery.tagsSelTeacher[1] 
          var param = {
            selTeacher: teacherid,
            teachtaskAcademicyear: that.tagslistQuery.tagsAcademicyear,
            teachtaskAcademicterm: that.tagslistQuery.tagsAcademicterm
          }
            ...
            if(response.code === 20000){ 
              ...
              */ 
              ...
              for(let i = 0; i < data.length ;i++ ){  
                if( data[i].tkTags != null){
                  var tags = data[i].tkTags.split(';')
                  for(let j = 0 ; j < tags.length ; j++){
                  
                    if(tagsmap.get(tags[j]) == null){
                      tagsmap.set(tags[j],1) 
                    }else{ 
                      tagsmap.set(tags[j], tagsmap.get(tags[j])+1 ) 
                    }
                  }
                } 
              ...
              var tagstmp = []    
              var tagsLegendData = [] 
              tagsmap.forEach(function(value , key){ 
                tagstmp.push({ name:key , value: value})  
                tagsLegendData.push(key)
              })  
              /**
                 0: {"示例" => 1}
                1: {"123" => 1}
              */  
              that.tagsData = tagstmp 

              that.tagsLegendData = tagsLegendData  
    ...
    initChart() {
      ...
      const xData = (async function() {
        await fetchTeacherList().then(response => {   
          ...
          that.tagsTeacherOptions = collegeArr
        }) 
      }())
    }
