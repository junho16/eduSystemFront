<template> 
  ...
  <!-- <div class="dashboard-editor-container"> -->
    ...
    >
      ...
      <el-table-column label="Actions" align="center" width="200" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          ...
          <el-button  width="200" type="info"  size="medium" @click="huifuMsg(row)">
            回复
          </el-button>  
        </template>
      </el-table-column>
    ...
    <el-dialog title="详情" :visible.sync="info_dialogTableVisible"> 
      ...
      <!-- <el-table :data="msgallinfo">
        <el-table-column property="msgId" label="编号" width="150"></el-table-column>
        <el-table-column property="bumen" label="部门" width="200"></el-table-column>
        <el-table-column property="msgTitle" label="主题" width="200"></el-table-column>
        <el-table-column property="studentId" label="提问者学号" width="200"></el-table-column>
        <el-table-column property="studentName" label="提问者姓名" width="200"></el-table-column>
        <el-table-column property="msgContent" label="提问内容" width="200"></el-table-column>
        <el-table-column property="msgCreatetime" label="提问时间" width="200"></el-table-column>
        <el-table-column property="msgIsreply" label="回复状态" width="200"></el-table-column> 
        <el-table-column property="msgReplycontent" label="回复内容" width="200"></el-table-column>
        <el-table-column property="msgReplytime" label="回复时间" width="200"></el-table-column> 
      </el-table> -->
    </el-dialog>  
    <el-dialog title="回复" :visible.sync="huifu_dialogFormVisible">
      <el-form >
        <el-form-item label="请键入回复">
          <el-input
            type="textarea"
            :rows="3"
            placeholder="请输入内容"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="huifu_dialogFormVisible = false">取 消</el-button>
        <el-button type="primary" @click="sendhuifu">确 定</el-button>
      </div>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: { 
    handleFilter(){
      ...
      var tmp = []
      if(this.bumen == '' && this.state == ''){
        this.showmsglist = this.msglist 
      }
      if(this.bumen == '' && this.state != ''){
        for(let i = 0 ; i < this.msglist.length ; i++){ 
          if(this.msglist[i].msgIsreply == this.state)
      ...
      if(this.bumen != '' && this.state != ''){
        for(let i = 0 ; i < this.msglist.length ; i++){ 
          if(this.msglist[i].bumen == this.bumen && this.msglist[i].msgIsreply == this.state )
            tmp.push(this.msglist[i])
        }
        this.showmsglist = tmp 
      }
    ...
    huifuMsg(row){
      this.selobj = Object.assign({}, row) 
      this.huifu_dialogFormVisible = true
    },
    sendhuifu(){
      var param = {
