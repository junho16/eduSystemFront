<template>
  ...
  <!-- <div class="dashboard-editor-container"> -->
    <div class="filter-container">
      ...
      <!-- 是否查看选修课程  开关 --> 
      <el-tooltip :content="'是否素质拓展课程'" placement="top" style="width:80px;margin-left:17px">
        <el-switch
          @change="isShowXX()"
          v-model="switchValue" 
          active-color="#13ce66"
          ...
          active-value="素质拓展课程"
          inactive-value="非素质拓展课程">
        </el-switch>
      </el-tooltip>


      <!-- <el-input v-model="listQuery.keyword" placeholder="课程名关键字" style="width: 160px;" class="filter-item" @keyup.enter.native="handleFilter" /> -->
     
        <!-- <el-option v-for="item in xyzybjOptions" :key="item.index" :label="item.label" :value="item.value" /> -->
     ...
     <!-- <el-select v-model="listQuery.teachtaskAcademicyear" placeholder="选择班级" clearable style="width:150px;margin-left:17px" class="filter-item">
        <el-option v-for="item in xyzybjOptions" :key="item.index" :label="item.label" :value="item.value" />
      </el-select>  -->
      <el-select v-model="listQuery.teachtaskAcademicyear" placeholder="学年" clearable style="width:150px;margin-left:17px" class="filter-item">
        <el-option v-for="item in teachtaskAcademicyearOptions" :key="item.index" :label="item.label" :value="item.value" />
      </el-select> 
      <el-select v-model="listQuery.teachtaskAcademicterm" placeholder="学期" clearable style="width:80px;margin-left:17px" class="filter-item">
        <el-option v-for="item in teachtaskAcademictermOptions" :key="item.index" :label="item.label" :value="item.value" />
      </el-select> 
      <el-select v-model="listQuery.teachtaskStarttime" placeholder="开课时间" clearable style="width:110px;margin-left:17px" class="filter-item">
        <el-option v-for="item in teachtaskStarttimeOptions" :key="item.index" :label="item.label" :value="item.value" />
      </el-select>
      <el-select v-model="listQuery.teachtaskEndtime" placeholder="截止时间" clearable style="width:110px;margin-left:17px" class="filter-item">
        <el-option v-for="item in teachtaskEndtimeOptions" :key="item.index" :label="item.label" :value="item.value" />
      </el-select>
      
      <el-select v-model="listQuery.teachtaskGradelimit" placeholder="限制年级" clearable style="width:110px;margin-left:17px" class="filter-item">
        <el-option v-for="item in teachtaskGradelimitOptions" :key="item.index" :label="item.label" :value="item.value" />
      </el-select>
      
        
      <!-- <el-select v-model="listQuery.teachtaskGradelimit" multiple placeholder="限制年级"  clearable style="width:110px;margin-left:17px" class="filter-item"> 
        <el-option
      ...
      <!-- <el-select v-model="listQuery.collegeid" placeholder="选择所属学院" clearable style="width:140px" class="filter-item">
        <el-option v-for="item in showColleges" :key="item.index" :label="item.label" :value="item.value" />
      </el-select> -->
      <!-- <el-select v-model="listQuery.kcxzid" placeholder="选择课程性质" clearable style="width:140px" class="filter-item">
        <el-option v-for="item in showKcxz" :key="item.index" :label="item.label" :value="item.value" />
      </el-select> -->
      <!-- <el-select v-model="listQuery.khfs" placeholder="选择考核方式" clearable style="width:140px" class="filter-item">
    ...
    >
      ...
      <el-table-column label="课程名称" width="170px" align="left">
        <template slot-scope="{row}">
          <span>{{ row.courseName }}</span>
        </template>
      </el-table-column>  
      <el-table-column label="课程所属院系" width="150px" align="left">
        <template slot-scope="{row}">
          <span>{{ row.collegeName }}</span>
        </template>
      </el-table-column>  
   <!-- <el-select v-model="listQuery." placeholder="" clearable style="width:110px;margin-left:17px" class="filter-item">
        <el-option v-for="item in teachtaskGradelimitOptions" :key="item.index" :label="item.label" :value="item.value" />
      </el-select> -->
        
      <el-table-column label="限制年级" width="170px" align="left" v-if="switchValue!='素质拓展课程'" >
        <template slot-scope="{row}">
          <span>{{ row.teachtaskGradelimit }}</span>
        </template>
      </el-table-column>  
      <el-table-column label="专业" width="170px" align="left" v-if="switchValue=='素质拓展课程'" >
        <template slot-scope="{row}">
          <span>{{ row.professionName }}</span>
        </template>
      </el-table-column>  
      <el-table-column label="班级" width="120px" align="left" v-if="switchValue=='素质拓展课程'" >
        <template slot-scope="{row}">
          <span>{{ row.classId }}</span>
        </template>
      </el-table-column>  
      <el-table-column label="课程申请状态" width="130px" align="center"> 
        <template slot-scope="{row}">
          <el-button type="primary" disabled v-if="row.teachtaskIsrank == 0" >未审核</el-button>
          <el-button type="success" disabled v-if="row.teachtaskIsrank == 1" >已审核</el-button> 
          <el-button type="danger" disabled v-if="row.teachtaskIsrank == 2" >已拒绝</el-button> 
          <!-- <template slot-scope="{row}">
            <span>{{ row.teachtaskIsrank }}</span>
          </template> -->
        </template>
      </el-table-column> 
      ...
      <el-table-column label="Actions" align="center" width="130" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          <el-button  type="success"  size="medium" @click="handleTeachtaskYes(row)">
            同意开课申请
          </el-button> 
        </template>
      </el-table-column>
      ...
      <el-table-column label="Actions" align="center" width="130" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          <el-button  type="danger"  size="medium" @click="handleTeachtaskNo(row)">
            拒绝开课申请
          </el-button> 
        </template>
      </el-table-column>
...
export default {
  ...
  methods: { 
    ...
    handleFilter() {
      ...
      // }
      var tmpp = this.listQuery
      var teacherTmp = tmpp.teacher
      if(teacherTmp!=null){
        this.listQuery.collegeid = teacherTmp[0]
        this.listQuery.teacherid = teacherTmp[1] 
        this.listQuery.teacher = null 
      }
      this.getList()
    }, 
    isShowXX(){
      if(this.switchValue!='素质拓展课程'){ 
        this.showteachtasklist = this.xxteachtasklist
      }else{
        this.showteachtasklist = this.zcteachtasklist
      }
    },
    // 选择开课的课程 打开面板进行开课
    handleChoose(row) { 
    ...
    handleTeachtaskYes(row){
      let id = row.teachtaskId  
      this.$confirm('同意此开课申请?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => { 
        handleTeachtaskStatus(id,"1").then(response => {
          if(response.code === 20000){
            this.getList()
            this.$alert('申请已通过', '操作提示', {
              confirmButtonText: '确定' 
            });
          }else{
            this.$alert('申请未通过，出错了！', '操作提示', {
              confirmButtonText: '确定'
            });
          }   
        })
    ...
    handleTeachtaskNo(row){
      let id = row.teachtaskId  
      this.$confirm('拒绝此开课申请?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => { 
        handleTeachtaskStatus(id,"2").then(response => {
          if(response.code === 20000){
            this.getList()
            this.$alert('已拒绝', '操作提示', {
              confirmButtonText: '确定' 
